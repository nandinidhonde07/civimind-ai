...
export default function CitizenDashboard() {
  ...
  const simulateAI = (text: string) => {
    ...
    return {
      ...
      priority,
      duplicateProbability: Math.floor(Math.random() * 30),
      estimatedResolution: priority === 'Critical' ? '4 hours' : priority === 'High' ? '12 hours' : '2 days',
      imageVerification: {
        validImage: hasImage,
    ...
    <div className="max-w-[1100px] w-full mx-auto px-6 space-y-8 animate-in fade-in duration-500">
      ...
      <Tabs value={isSubmitted ? "history" : undefined} defaultValue="submit" className="w-full">
        <TabsList className="grid w-full grid-cols-3 max-w-[500px] bg-white p-1.5 rounded-xl border border-slate-200 shadow-sm h-12">
          <TabsTrigger value="submit" onClick={() => setIsSubmitted(false)} className="rounded-lg data-[state=active]:bg-[#DEEBF7] data-[state=active]:text-[#6BAED6] data-[state=active]:font-bold font-medium text-slate-500 transition-all">Submit Issue</TabsTrigger>
        ...
        <TabsContent value="history" className="mt-7 w-full">
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
             ...
             {isSubmitted && aiResult && (
               <Card className="bg-white border-slate-200 rounded-3xl overflow-hidden shadow-lg mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                       ...
                       <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-inner">
                         ...
                         <div className="flex items-center gap-2 text-sm text-slate-600 flex-wrap bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                           {aiResult.departmentsRequired?.map((dept: string, i: number) => (
            ...
            <Card className="bg-white border-slate-200 shadow-md rounded-3xl">
              <CardHeader className="p-8 pb-6 border-b border-slate-100 bg-[#F7FBFF] rounded-t-3xl flex flex-row justify-between items-center">
                ...
                {latestComplaint && <Badge className="bg-slate-100 text-slate-600 border-none shadow-none text-sm font-bold">{latestComplaint.id}</Badge>}
              </CardHeader>
              <CardContent className="p-8">
                  ...
                  <>
                    ...
                    <div className="w-full space-y-8 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-300 before:to-transparent">
                       ...
                       {latestComplaint.timeline.map((event, index) => {
                         ...
                         if (event.status === 'AI Verified' || event.status === 'Department Assigned') {
                           Icon = BrainCircuit;
                         } else if (event.status.includes('Progress') || event.status.includes('Accepted') || event.status === 'Pending Triage') {
                           Icon = Clock;
                           if (isLast && latestComplaint.status !== 'Resolved') colorClass = "bg-white text-[#F59E0B] shadow-[0_0_0_2px_#F59E0B] border-white";
                         ...
                         return (
                           <div key={index} className={`relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group ${isLast ? 'is-active' : ''}`}>
                             <div className={`flex items-center justify-center w-10 h-10 rounded-full border-4 shadow-md shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 ${colorClass}`}>
                               <Icon className={`w-5 h-5 ${isLast && !event.status.includes('Resolved') ? 'animate-pulse' : ''}`} />
                             </div>
                             <div className={`w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white p-5 rounded-2xl border ${isLast && !event.status.includes('Resolved') ? 'border-[#F59E0B] bg-orange-50' : 'border-slate-200'} shadow-sm text-left relative overflow-hidden`}>
                               {isLast && !event.status.includes('Resolved') && <div className="absolute top-0 left-0 w-1 h-full bg-[#F59E0B]"></div>}
                               ...
                               <div className="flex items-center justify-between mb-2">
                                 ...
                                 {isLast && !event.status.includes('Resolved') && <Badge className="bg-[#F59E0B] text-white px-2 py-0.5 text-[10px]">Active</Badge>}
